...
def anchura(laberinto, nodoActual, objetivox, objetivoy, personaje):
        ...
        ##Para evitar hacer esta cosa eterna con recursividad mejor le meto un
        ## while la queue siga viva :P
        while queue:
            
            nodoActual = queue.popleft()
            visitados.append([nodoActual.posx, nodoActual.posy])
            if (nodoActual.posx, nodoActual.posy) == (objetivox, objetivoy):
                print("Si sirvio")
                path = []
                while nodoActual:
                    path.append((nodoActual.posx, nodoActual.posy))
                    nodoActual = nodoActual.padre
                return path[::-1]
        
            ##Derecha
            if nodoActual.posy < cols - 1 and laberinto[nodoActual.posx][nodoActual.posy + 1] in personaje.terr_disp:
                nodo = TreeNode(nodoActual.posx, nodoActual.posy + 1, 'r', nodoActual)
                coords = [nodo.posx, nodo.posy]
                if coords not in visitados:
                    queue.append(nodo)
                    nodoActual.add_child(nodo)
                    visitados.append(coords)
            ##Abajo
            if nodoActual.posx < rows - 1 and laberinto[nodoActual.posx+1][nodoActual.posy] in personaje.terr_disp:
                nodo = TreeNode(nodoActual.posx + 1,
                                nodoActual.posy, 'd', nodoActual)
                coords = [nodo.posx, nodo.posy]
                if coords not in visitados:
                    queue.append(nodo)
                    nodoActual.add_child(nodo)
                    visitados.append(coords)
            ##Arriba    
            if nodoActual.posx > 0 and laberinto[nodoActual.posx-1][nodoActual.posy] in personaje.terr_disp:
                nodo = TreeNode(nodoActual.posx-1,
                                nodoActual.posy, 'u', nodoActual)
                coords = [nodo.posx, nodo.posy]
                if coords not in visitados:
                    queue.append(nodo)
                    nodoActual.add_child(nodo)
                    visitados.append(coords)
            ##Izq
            if nodoActual.posy > 0 and laberinto[nodoActual.posx][nodoActual.posy - 1] in personaje.terr_disp:
                nodo = TreeNode(nodoActual.posx,
                                nodoActual.posy-1, 'l', nodoActual)
                coords = [nodo.posx, nodo.posy]
                if coords not in visitados:
                    queue.append(nodo)
                    nodoActual.add_child(nodo)
